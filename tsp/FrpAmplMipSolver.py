...
class FrpAmplMipSolver(solver.Solver):
    ...
    def solve(self, prob, M, P): 
        if type(prob) == frp.FastRouteProb:
            ...
            ampl.setOption('gurobi_options', 'timelim 1200 outlev 1')
            dir_ampl = os.path.normpath('C:/Users/David/Desktop/equipe3_TP2')
            ampl.read(os.path.join(dir_ampl, 'tsp_ampl_model.mod'))
            ...
            df = amplpy.DataFrame('Y')
            df.setColumn('Y', liste_Y)
            ampl.setData(df, 'Y')
            ...
            ampl.setData(df, 'J')
            df = amplpy.DataFrame('T')
            df.setColumn('', liste_J)
            ampl.setData(df, 'J')
            df = amplpy.DataFrame(('I', 'J'), 'Dist_Matrice')
            df.setValues({
                (T,I,J): dist_matrix[t][i][j]
                for t, T in enumerate(liste_T)
            ...
            for i in x.val:
                if i == 1:
                    coord = [x.index0[count], x.index1[count]]
                    visit_sequence_temp.append(coord)
                count = count + 1
