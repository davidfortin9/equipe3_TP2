...
param n >= 0; #Quantité de noeuds
param K >= 0; #Quantité de picker disponible
param C >= 0; #Capacité de transport par picker

set N := 0..n; # Ensemble du nombre de noeuds
set A := {i in N, j in N: i <> j}; # Ensemble des pair (i,j) ou i est différent de j

param c{A} >= 0; # Matrice des distance entre i et j
param d{1..n}; # Quantité de stock au noeud i
...
s.t. sub_routine {i in 1..n, j in 1..n: i <> j}: u[j] - u[i] + C*x[i,j] <= C - d[i]; # assure que le chemin reste continu du depart a l'arrive

s.t. Limites {i in 1..n}: d[i] <= u[i] <= C; # determine les limites parcouru par le vehicule